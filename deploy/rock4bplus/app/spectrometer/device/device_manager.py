...
class DeviceManager(QtCore.QObject):
    ...
    def __init__(self):
        ...
        self._identity_index: Dict[str, int] = {}
    ...
    def remove_device(self, device_id: int, preserve_probe_backoff: bool = False):
        ...
        for serial, indexed_id in list(self._identity_index.items()):
            if indexed_id == device_id:
                self._identity_index.pop(serial, None)
    ...
    def _on_response(self, device_id: int, cmd: int, params: bytes):
        ...
        if cmd == CmdCode.GET_VERSION and len(params) >= 32:
            ...
            if value and not set(value) <= {"0"}:
                ...
                self._reconcile_production_serial(device_id, value)
            else:
                device.identity_note = "设备未返回有效生产序列号"
                self.diagnostic_event.emit(
                    f"{device.port_name} 未返回有效生产序列号，当前连接位置仅用于诊断"
                )
        if device_id in self._announced_devices:
            self.device_updated.emit(device_id)
    ...
    def _reconcile_production_serial(self, device_id: int, serial: str) -> None:
        device = self.devices.get(device_id)
        if not device:
            return
        key = serial.strip().upper()
        previous_id = self._identity_index.get(key)
        if previous_id is None or previous_id == device_id:
            self._identity_index[key] = device_id
            device.identity_conflict = False
            device.identity_note = ""
            return
        previous = self.devices.get(previous_id)
        if previous is None:
            self._identity_index[key] = device_id
            return
        if previous.connected:
            ...
            note = f"生产序列号 {serial} 与设备 {previous_id} 重复"
            device.identity_note = note
            previous.identity_note = note
            self.diagnostic_event.emit(note)
            self.device_updated.emit(previous_id)
            return

        old_port = previous.port_name
        device.previous_port_name = old_port
        device.identity_note = f"从 {old_port} 重新枚举到 {device.port_name}"
        self.diagnostic_event.emit(
            f"光谱仪 {serial} 已从 {old_port} 重新枚举到 {device.port_name}"
        )
        self.remove_device(previous_id)
        self._identity_index[key] = device_id
